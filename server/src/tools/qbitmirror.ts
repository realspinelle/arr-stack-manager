...
import type { InstanceConfig, SyncOptions, TargetSyncStats, TorrentInfo } from "../types/qbit";
...
export async function runQbitMirror() {

}

export async function initQbitMirror() {
    await addSetting("qbitmirror.enabled", "false");
    await addSetting("qbitmirror.main", ""); // id in db for the "main" qbit
    await addTask("qbitmirror", "*/5 * * * *");
}